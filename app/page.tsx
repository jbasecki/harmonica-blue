...
function DiscoverySanctuary() {
  ...
  return (
    <main style={{ height: '100vh', width: '100vw', background: '#000', color: '#D4AF37', fontFamily: 'serif', overflow: 'hidden', position: 'relative' }}>
      ...
      {/* 2. ATMOSPHERIC BACKGROUND WITH ORIGINAL SOUND */}
      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
        <video 
          key={bgIndex} autoPlay loop playsInline
          // Removed 'muted' to play original nature sounds
      ...
      {/* 3. THE GALLERY FRAME (Upper Corners) */}
      <div style={{ position: 'absolute', top: '3vh', left: '4vw', zIndex: 3 }}>
        <p style={{ fontSize: '0.45rem', fontStyle: 'italic', letterSpacing: '1px', opacity: 0.4 }}>
          Visual by {BG_CREDITS[bgIndex]}
        </p>
      </div>

      <div style={{ position: 'absolute', top: '3vh', right: '4vw', zIndex: 3, textAlign: 'right' }}>
        <div style={{ display: 'inline-block', width: '20px', height: '12px', border: '0.5px solid #D4AF37', marginBottom: '3px' }}></div>
        <p style={{ fontSize: '0.35rem', letterSpacing: '2px', opacity: 0.5 }}>harmonica-blue.app</p>
      </div>

      {/* 4. THE INTEGRATED UI OVERLAY */}
      <div style={{ position: 'relative', zIndex: 2, height: '100%', width: '100%', display: 'flex', flexDirection: 'column', padding: '5vh 6vw', boxSizing: 'border-box' }}>
        ...
        {/* LOWER DASHBOARD: NAMES & CONTROLS ALIGNED */}
        <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', border: '0.3px solid rgba(212,175,55,0.2)', padding: '10px', borderRadius: '6px', width: '170px' }}>
            ...
            <div style={{ marginTop: '8px', height: '22px', border: '0.5px solid #D4AF37', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.5rem', cursor: 'pointer' }}>PLAY GIFT</div>
          </div>

          {/* CENTER: GREETING & PICKER */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', maxWidth: '350px', marginBottom: '5px' }}>
            ...
            <button style={{ background: '#D4AF37', color: '#000', padding: '8px 25px', borderRadius: '15px', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: '0.6rem', letterSpacing: '2px', marginTop: '10px' }}>
              STASH & SEND
            </button>
          </div>
