...
const CURSIVE_FONTS = [
  "'Dancing Script', cursive", // Option 1
  "'Great Vibes', cursive"      // Option 2
];
...
function DiscoverySanctuary() {
  ...
  const [fontChoice, setFontChoice] = useState(0); // Toggle between the 2 fonts
  ...
  return (
    <main style={{ height: '100vh', width: '100vw', background: '#000', color: '#D4AF37', overflow: 'hidden', position: 'relative' }}>
      ...
      {/* BACKGROUND FIX: Muted ensures the video actually plays */}
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', width: '100%', display: 'flex', flexDirection: 'column', padding: '5vh 6vw', boxSizing: 'border-box' }}>
        <div style={{ flex: 1.2, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '15px' }}>
          ...
          <div 
            onClick={() => setFontChoice(fontChoice === 0 ? 1 : 0)} // Click name to toggle font
            style={{ 
              fontFamily: CURSIVE_FONTS[fontChoice], 
              fontSize: '2.4rem', color: '#D4AF37', 
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: '2px' }}>
              {[...Array(19)].map((_, i) => (
                <button key={i} onClick={() => setBgIndex(i)} style={{ width: '14px', height: '12px', background: bgIndex === i ? '#D4AF37' : 'none', border: '0.3px solid #D4AF37', color: bgIndex === i ? '#000' : '#D4AF37', fontSize: '0.28rem' }}>{i + 1}</button>
              ))}
            </div>
            <button style={{ background: '#D4AF37', color: '#000', padding: '6px 20px', borderRadius: '15px', fontWeight: 'bold', fontSize: '0.5rem' }}>STASH & SEND</button>
          </div>
