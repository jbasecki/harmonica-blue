...
export function DiscoverySanctuary({ initialData }: SanctuaryProps) {
  ...
  const [youtubeUrl] = useState(initialData?.youtubeUrl || '');
  ...
  // LOGIC: First and second-to-last letter for the top tiles
  ...
  const handleStashAndSend = async () => {
    ...
    } catch (e) { console.error("Stash error:", e); }
    setIsSaving(false);
  };
  ...
  return (
    <main style={{ height: '100vh', width: '100vw', background: '#000', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {/* TITLE */}
        <h1 style={{ marginTop: '5vh', letterSpacing: '18px', color: '#D4AF37', fontSize: '0.9rem', fontWeight: 300, opacity: 0.8 }}>HARMONICA</h1>

        {/* TOP ANCHOR: Visual Tiles and Name Bridge */}
        <div style={{ marginTop: '4vh', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          <div style={{ display: 'flex', gap: '10px' }}>
            {getNameTiles(toName).map((ltr, i) => (
              <img key={i} src={`${bucketUrl}/${ltr}5.png`} style={{ width: '48px', border: '0.4px solid #D4AF37', borderRadius: '4px' }} alt="" />
            ))}
          </div>
        ...
        {/* THE GLASS VESSEL: CENTERED, LARGE, & MARGIN-FIXED */}
        <div style={{ 
          ...
          padding: '40px', boxSizing: 'border-box' // PREVENTS OVERFLOW
        }}>
          <textarea 
            disabled={isReceiver}
            value={text} 
            onChange={(e) => setText(e.target.value)}
            placeholder="Stash your wisdom..."
            style={{ 
              width: '100%', height: '100%', background: 'transparent', border: 'none', 
              textAlign: 'center', // HORIZONTAL CENTERING
              fontSize: '2.4rem', // BOLD VINTAGE SIZE
              fontFamily: cursiveFont, color: '#D4AF37', outline: 'none', resize: 'none',
              padding: '10px', scrollbarWidth: 'thin' // VERTICAL SCROLLBAR
            }} 
          />
          ...
          {shareableLink && (
            <div style={{ marginTop: '15px', fontSize: '0.6rem', color: '#D4AF37', background: 'rgba(0,0,0,0.3)', padding: '5px 15px', borderRadius: '10px' }}>
              READY: {shareableLink}
            </div>
          )}
        </div>
        ...
        {!isReceiver && (
          <div style={{ position: 'absolute', bottom: '3vh', display: 'flex', gap: '12px' }}>
            {[...Array(10)].map((_, i) => (
              <button key={i} onClick={() => setBgIndex(i)} style={{ width: '30px', height: '2px', background: bgIndex === i ? '#D4AF37' : 'rgba(212, 175, 55, 0.15)', border: 'none', cursor: 'pointer', transition: '0.3s' }} />
            ))}
          </div>
        )}
      </div>
